...
type serviceEntrySyncer struct {
	...
	lock         sync.RWMutex
	entries      map[string]*istiov1a3.ServiceEntry
...
func newServiceEntrySyncer(c client.Client, logger *logr.Logger) *serviceEntrySyncer {
	s := &serviceEntrySyncer{
		client:       c,
		logger:       logger,
		entries:      make(map[string]*istiov1a3.ServiceEntry),
...
func (syncer *serviceEntrySyncer) getFromK8s(ctx context.Context, service string, se *istiov1a3.ServiceEntry) error {
...
func (syncer *serviceEntrySyncer) deleteFromK8s(ctx context.Context, se *istiov1a3.ServiceEntry) {
	c := syncer.client
...
func (syncer *serviceEntrySyncer) addToK8s(ctx context.Context, service string, entry *istioapi.ServiceEntry) *istiov1a3.ServiceEntry {
	c := syncer.client
	ns := config.RootNamespace()
	se := istiov1a3.ServiceEntry{
		Spec: *entry.DeepCopy(),
	}
	se.Namespace = ns
	if se.Labels == nil {
...
func (syncer *serviceEntrySyncer) updateToK8s(ctx context.Context, se *istiov1a3.ServiceEntry, entry *istioapi.ServiceEntry) *istiov1a3.ServiceEntry {
...
func (syncer *serviceEntrySyncer) Update(ctx context.Context, entries map[string]*istioapi.ServiceEntry) {
	...
	var obj istiov1a3.ServiceEntry
	...
	var latestServiceEntry *istiov1a3.ServiceEntry
...
func (syncer *serviceEntrySyncer) sync() {
	...
	persisted := make(map[string]*istiov1a3.ServiceEntry, len(serviceEntries.Items))
