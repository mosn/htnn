...
var _ = Describe("FilterPolicy controller, for gateway", func() {
	...
	Context("When generating LDS plugin configuration via ECDS (Istio Gateway)", func() {
		...
		It("deal with Istio gateway", func() {
			...
			mustReadFilterPolicy("istio_gateway", &input)
		...
		It("deal with Istio gateway via port", func() {
			...
			mustReadFilterPolicy("istio_gateway_via_port", &input)
		...
		It("deal with embedded FilterPolicy", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("istio_gateway_embedded_policy", &input)
			...
			for _, cp := range ef.Spec.ConfigPatches {
				...
				Expect(info.FilterPolicies).To(ConsistOf([]string{"default/embedded-gateway-default-embedded"}))
			}
		})
		...
		It("deal with embedded FilterPolicy, ignore invalid embedded FilterPolicy", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("istio_gateway_embedded_invalid_policy", &input)
	...
	Context("When generating LDS plugin configuration via ECDS (k8s Gateway)", func() {
		...
		It("deal with k8s gateway", func() {
			...
			mustReadFilterPolicy("gateway", &input)
		...
		It("deal with k8s gateway via port", func() {
			...
			mustReadFilterPolicy("gateway_via_port", &input)
