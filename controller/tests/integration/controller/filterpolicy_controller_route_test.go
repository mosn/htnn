...
var _ = Describe("FilterPolicy controller, for route", func() {
	...
	Context("When reconciling FilterPolicy with VirtualService", func() {
		var (
		...
		BeforeEach(func() {
			input := []map[string]interface{}{}
			mustReadFilterPolicy("default_istio", &input)
		...
		It("deal with virtualservice", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("virtualservice", &input)
			...
			// delete virtualservice referred by filterpolicy
		...
		It("deal with multi policies to one virtualservice", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("multi_policies", &input)
		...
		It("diff envoyfilters", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("diff_envoyfilters", &input)
		...
		It("refer virtualservice across namespace", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("refer_virtualservice_across_namespace", &input)
		...
		It("route doesn't match", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("virtualservice_match_but_route_not", &input)
			for _, in := range input {
				obj := pkg.MapToObj(in)
				Expect(k8sClient.Create(ctx, obj)).Should(Succeed())
			}
		...
		It("deal with virtualservice via route name", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("virtualservice_via_route_name", &input)
		...
		It("deal with sub-policies to one virtualservice", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("virtualservice_subpolicies", &input)
		...
		It("deal with embedded FilterPolicy", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("virtualservice_embedded_policy", &input)
			...
			for _, ef := range envoyfilters.Items {
				...
				if ef.Name == "htnn-h-default.local" {
					...
					Expect(info.FilterPolicies).To(ConsistOf([]string{"default/embedded-virtualservice-vs"}))
				}
			}
			Expect(names).To(ConsistOf([]string{"htnn-http-filter", "htnn-h-default.local"}))
			...
			// update annotation
			virtualService.Annotations[constant.AnnotationFilterPolicy] = `{"apiVersion":"htnn.mosn.io/v1","kind":"FilterPolicy","metadata":{"name":"policy","namespace":"default"},"spec":{"filters":{"demo":{"config":{"hostName":"Zhang"}}}}}`
			err = k8sClient.Update(ctx, virtualService)
			Expect(err).NotTo(HaveOccurred())
			Eventually(func() bool {
				if err := k8sClient.List(ctx, &envoyfilters); err != nil {
		...
		It("deal with embedded FilterPolicy, ignore invalid embedded FilterPolicy", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("virtualservice_embedded_invalid_policy", &input)
		...
		It("deal with embedded FilterPolicy with SubPolicies", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("virtualservice_embedded_policy_with_subpolicies", &input)
	...
	Context("When reconciling FilterPolicy with HTTPRoute", func() {
		var (
		...
		It("deal with httproute", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("httproute", &input)
			...
			// delete httproute referred by filterpolicy
		...
		It("deal with unattached httproute", func() {
			ctx := context.Background()
			input := []map[string]interface{}{}
			mustReadFilterPolicy("httproute_without_gateway", &input)
