...
func createResource(ctx context.Context, policy *mosniov1.FilterPolicy, virtualService *istiov1a3.VirtualService, i int) {
	id := strconv.Itoa(i)
...
var _ = Describe("FilterPolicy controller", func() {
	BeforeEach(func() {
	...
	Context("test reconciliation performance", func() {
		It("standard case", func() {
			// Each VirtualService has two routes, and three FilterPolicies
			input := []map[string]interface{}{}
			mustReadInput("filterpolicy", &input)
			...
			for _, in := range input {
				...
				} else if gvk.Group == "htnn.mosn.io" && gvk.Kind == "FilterPolicy" {
					policy = obj.(*mosniov1.FilterPolicy)
				} else {
