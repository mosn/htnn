...
func init() {
	suite.Register(suite.Test{
		...
		Run: func(t *testing.T, suite *suite.Suite) {
			...
			// test webhook
			gw.SetAnnotations(map[string]string{constant.AnnotationFilterPolicy: "invalid"})
			err = c.Update(ctx, &gw)
			require.ErrorContains(t, err, "configuration is invalid: cannot unmarshal FilterPolicy: ")

			incorrectPolicy := `{"apiVersion":"htnn.mosn.io/v1","kind":"FilterPolicy","metadata":{"name":"policy"},"spec":{"filters":{"demo":{"config":{"hostName":"micky"}}},"subPolicies":[{"sectionName":"gw","filters":{"demo":{"config":{"hostName":["doraemon"]}}}}]}}`
			gw.SetAnnotations(map[string]string{constant.AnnotationFilterPolicy: incorrectPolicy})
			err = c.Update(ctx, &gw)
			...
			gw.SetAnnotations(map[string]string{constant.AnnotationFilterPolicy: incorrectWhenValidateStrictlyPolicy})
			err = c.Update(ctx, &gw)
			require.ErrorContains(t, err, "configuration is invalid: invalid FilterPolicy: ")
		},
	})
