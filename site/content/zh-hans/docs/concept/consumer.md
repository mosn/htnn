...
我们可以在消费者的 `auth` 字段下配置[消费者插件](../../developer-guide/plugin_development)。每种消费者插件都会有两种配置：一种配置在路由上，负责指定认证参数的来源，比如上面的 FilterPolicy 的 `keys`。另一种配置在消费者上，负责指定怎样的认证参数匹配到所在的消费者，比如上面的 Consumer 的 `key`。
